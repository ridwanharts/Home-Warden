      ...
      {!result && (
        <section className="space-y-4 py-4">
          <div className="text-center space-y-2">
            <div className="mx-auto w-12 h-12 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400">
              <Sparkles size={24} />
            </div>
            <div className="space-y-1">
              <h2 className="text-xl font-bold tracking-tight uppercase">DIY Repair Mentor</h2>
              <p className="text-slate-500 text-xs italic">Diagnosis & Panduan Langkah-demi-Langkah</p>
            </div>
          </div>
      ...
      {result && (
        ...
        >
          ...
          <div className="bg-gradient-to-br from-indigo-600/10 to-transparent border border-indigo-500/20 p-5 rounded-2xl relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-slate-100 mb-3 leading-tight italic">"{result.problem}"</h3>
            <p className="text-slate-400 leading-relaxed text-xs">{result.diagnosis}</p>
          </div>
          ...
          {result.safetyWarning && (
            <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="text-rose-500 font-bold text-[10px] uppercase tracking-widest">Protocol Warning</h4>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="glass-card !bg-slate-900/60 p-4">
              <h4 className="flex items-center gap-3 text-xs font-bold uppercase text-slate-400 tracking-widest mb-3">
            ...
            <div className="glass-card !bg-slate-900/60 p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {result.toolsNeeded.map((tool, i) => (
                  <span key={i} className="px-2.5 py-1 rounded-lg bg-slate-800 text-[10px] font-medium text-slate-300 border border-slate-700/50">
                    {tool}
                  </span>
                ))}
              </div>
